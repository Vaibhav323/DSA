...
template <class T> Stack<T>::Stack(T value) {
  top = -1;
  top++;
  data[top] = value;
}
...
template <class T> void Stack<T>::onTop() {
  std::cout << ">>\tTopmost value in stack is " << data[top] << std::endl;
}

template <class T> void Stack<T>::size() {
  int temp = top;
  std::cout << ">>\tSize of stack is " << ++temp << std::endl;
}
template <class T> bool Stack<T>::isEmpty(dt_STACK top_val) {
  return (top_val == -1);
}

template <class T> bool Stack<T>::isFull(dt_STACK top_val) {
